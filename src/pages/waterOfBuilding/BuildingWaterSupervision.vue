    ...
    <div class="content body">
      <div class="water-charts">
        ...
        <div class="box box-solid">
          <div class="box-header bg-aqua">建筑用水监测图表</div>
          <div class="box-body">
            <div class="water-charts-selectDate">
              <select-building class="select-building" :buildingId="buildingId" @change="link"></select-building>
      ...
      <div class="water-table">
        <div class="box box-solid">
          <div class="box-header bg-aqua">建筑用水监测数据<el-button size="mini" type="success" icon="el-icon-download" @click="exportData">导出</el-button></div>
          <div class="box-body">
...
<script>
  ...
  import WaterChartsLine from "./buildingWaterSupervision/waterChartsLine";
  import WaterTable from "./buildingWaterSupervision/WaterTable";
  ...
  export default {
    ...
    methods: {
      ...
      async findFlowByYear() {
        ...
        let data = await this.$api.buildingData.findTimesYearData(params);
        data = data && data.data && data.data.list && data.data.list.length > 0 ? data.data.list : false;
        let flow = await this.$api.buildingData.findTimesYearFlow(params);
      ...
      async findFlowByMonth() {
        ...
        let data = await this.$api.buildingData.findTimesMonthData(params);
        data = data && data.data && data.data.list && data.data.list.length > 0 ? data.data.list : false;
        data ? data.shift() : false;
        let flow = await this.$api.buildingData.findTimesMonthFlow(params);
        flow = flow && flow.data && flow.data.list && flow.data.list.length > 0 ? flow.data.list : false;
        params.starttime = this.compareStart;
        params.endtime = this.compareEnd;
        let compare = await this.$api.buildingData.findTimesMonthFlow(params);
      ...
      async findFlowByDay() {
        ...
        let data = await this.$api.buildingData.findTimesDayData(params);
        data = data && data.data && data.data.list && data.data.list.length > 0 ? data.data.list : [];
        data ? data.shift() : [];
        let flow = await this.$api.buildingData.findTimesDayFlow(params);
        flow = flow && flow.data && flow.data.list && flow.data.list.length > 0 ? flow.data.list : [];
        params.starttime = this.compareStart;
        params.endtime = this.compareEnd;
        let compare = await this.$api.buildingData.findTimesDayFlow(params);
      ...
      async findFlowByHour() {
        ...
        let data = await this.$api.buildingData.findTimesHourData(params);
        data = data && data.data && data.data.list && data.data.list.length > 0 ? data.data.list : false;
        data ? data.shift() : false;
        let flow = await this.$api.buildingData.findTimesHourFlow(params);
        flow = flow && flow.data && flow.data.list && flow.data.list.length > 0 ? flow.data.list : false;
        this.getChartsData(flow,[]);
        this.getTableData(flow,data,[]);
      },
...
<style scoped>
  @import "../../../static/css/buildingWaterSupervision.css";
</style>
