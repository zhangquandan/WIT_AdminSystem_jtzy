  ...
  <div>
    <el-dialog
      ...
      <div class="receipt-table">
        <div class="top-header">漏损勘察回单</div>
        <div class="top-body">
          <table class="receipt-top" border="1" width="100%">
            ...
            <tr>
              <td>处理结果</td>
              <td colspan="2">{{tableData.disposalOpinion ? '需转派' : '无需转派'}}</td>
              <td>出单人</td>
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" size="medium">
            <div>
              <el-form-item label="验收意见" prop="checkOpinion">
                <el-select size="mini" v-model="ruleForm.checkOpinion" clearable>
                  <el-option label="同意" value="1"></el-option>
                  <el-option label="不同意" value="0"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="验收人" prop="checker">
                <el-input size="mini" v-model="ruleForm.checker" disabled></el-input>
              </el-form-item>
            </div>
            <div>
              <el-form-item label="验收时间" prop="checkTime">
                <el-input size="mini" v-model="ruleForm.checkTime" clearable>
    ...
    export default {
      ...
      methods: {
        ...
        handleClose(done) {
          this.$confirm('确认关闭？')
            .then(_ => {
              this.$emit('changeSurveyReceipt',{receiptFlag: false})
              done();
            })
            .catch(_ => {});
        },
            ...
            if (valid) {
              ...
              this.$api.alarm.updateAlarmDispatch(data).then(res => {
                if(res.status == 200) {
                  ...
                  this.$emit('changeSurveyReceipt',{receiptFlag: false,type: 1})
                }else {
                  this.$message({
                    type: 'error',
                    message: '你还没有开通此操作权限！'
...
<style scoped>
  .receipt-table {height: 700px;width: 660px;display: flex;flex-flow: column;}
  .top-header,.bottom-header {padding: 10px 0px 10px 10px;width: 100%;background: deepskyblue;}
  .top-body{flex: 3;border: 1px solid deepskyblue;padding: 10px;display: flex;align-items: center;}
  .bottom-body {flex: 1;border: 1px solid deepskyblue;padding: 10px;display: flex;flex-flow: column;align-items: center;justify-content: space-evenly;}
