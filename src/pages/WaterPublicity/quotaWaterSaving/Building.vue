<template>
    <div class="building-body" v-loading="show">
      <div class="building-radio">
  ...
  export default {
    ...
    methods: {
      ...
      async refresh() {
        ...
        this.tableData = [];
        var quota = await this.$api.buildingInf.findALL();
        if(quota) {
          quota.forEach((item) => {
      ...
      async getTableData(item,length) {
        var params = {pageNum: 1,pageSize: 1000,buildingid: item.id,yy: this.year};
        var flow = await this.$api.buildingInf.findFlowPageYearMonth(params);
        params = {quotayear: this.year,buildingid: item.id};
        var quota = await this.$api.buildingQuota.findAll(params);
        ...
        if(typeof item == 'object') {
          ...
          ratio = usedQuota > 0 && usedWater > 0 ? ((saving/usedQuota)*100).toFixed(1) + '%' : '--';
          usedQuota = Number(usedQuota.toFixed(1));
          usedWater = Number(usedWater.toFixed(1));
          planQuota = Number(planQuota.toFixed(1));
        }
        quotaDepartment = quota && quota.editordepartment ? quota.editordepartment : '--';
        quotaPersonLiable = quota && quota.editor ? quota.editor : '--';
