  ...
  export default {
    ...
    methods: {
      ...
      getQuotaData(){
        ...
        waterQuota.buildingid = this.buildingId ;
      ...
      /*保存定额*/
      keep() {
        this.$confirm('请确认是否保存当前建筑年度定额?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(()=>{
          let buildingWaterquota = this.qs.stringify(this.getQuotaData());
          if(this.status == 0){
            this.$api.buildingQuota.addQuota(buildingWaterquota).then(res => {
              if(res.msg == 'ok'){
            ...
            this.$api.buildingQuota.updateQuota(buildingWaterquota).then(res => {
              if(res.msg == 'ok'){
      ...
      async init() {
        ...
        let quota = await this.$api.buildingQuota.findAll(params);
        quota = quota.data && typeof quota.data == 'object' && quota.data.length > 0 ? quota.data[0] : {};
        if(quota.id) {
          this.status = 1;
          this.id = quota.id;
          this.buildingName = quota.buildingname;
        }else {
          this.status = 0;
        }
        params.quotayear = this.history;
        let historyQuota = await this.$api.buildingQuota.findAll(params);
        ...
        let flow = await this.$api.buildingInf.findFlowPageYearMonth(params1);
      ...
      getCurrentQuota() {
        ...
        this.$api.buildingQuota.findAll(params).then(res => {
      ...
      getHistoryFlow() {
        ...
        this.$api.buildingInf.findFlowPageYearMonth(params).then(res => {
