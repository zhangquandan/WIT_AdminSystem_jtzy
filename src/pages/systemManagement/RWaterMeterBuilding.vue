<template>
  <div class="flex">
    ...
    <div class="content">
      <div class="box">
        <div class="box-header bg-aqua">建筑计量水表管理</div>
        <div class="box-body">
          <div style="display:inline-flex;flex-flow:row;margin-bottom:10px;">
            <select-building @change="change" :buildingId="buildingId"></select-building>&emsp;
  ...
  export default {
    ...
    created(){
      ...
      if(params.buildingId != undefined){
        ...
        this.buildingValue = params.buildingId ;
      }
      this.refresh();
    },
    methods:{
      exportExcel () {
        ...
        try {
          this.fileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '建筑计量水表信息.xlsx')
      ...
      refresh(){
        ...
        this.$api.buildingInf.findAllRelation(params).then(res => {
          this.tableData = res;
        })
      },
      /*修改水表计量关系*/
      changeIsIn(row){
        this.$confirm('确定要修改水表计量关系？', '提示', {
          ...
          var rWatermeterBuilding  = {
            isin : row.isin ? 1:0,
            id : row.id,
            watermeterid : '',
            buildingid : row.buildingid
          };
        /*  rWatermeterBuilding  = this.qs.stringify(rWatermeterBuilding );*/
          this.$api.buildingInf.updateRelation(rWatermeterBuilding ).then(res => {
            if(res.msg == 'ok'){
              this.$message({
                type: 'success',
                message: '修改成功！'
      ...
      submitForm() {
        ...
        var rWatermeterBuilding   = this.ruleForm;
        if(this.ruleForm.watermeterid == null || this.ruleForm.watermeterid == ''){
          this.$alert('请从列表中选择水表','提示');
        }else{
          this.$api.buildingInf.addRelation(rWatermeterBuilding).then(res => {
            if(res.msg === 'ok'){
      ...
      handleDelete(index,row){
        this.$confirm('此操作将永久删除该内容, 是否继续?', '提示', {
          ...
          this.$api.buildingInf.deleteRelation(row.id)
            .then(res => {
              if(res.msg == 'ok'){
                this.$message({
                  type: 'success',
