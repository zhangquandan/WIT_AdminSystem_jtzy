<template>
  <div class="total-meter-table">
    ...
    <el-table v-if="show" border :data="totalData" height="100%" width="100%" size="mini" :header-row-style="rowStyle"  :row-style="rowStyle">
      ...
      <el-table-column v-for="(item,index) in tableColumn" :label="item.name" :prop="item.prop" :key="item.id" align="center" min-width="150">
      </el-table-column>
      <el-table-column
        align="center"
        label="流进/流出">
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      this.getTotalWaterMeter(param);
    },
    methods: {
      /*获取校区总计量水表*/
      getTotalWaterMeter(param){
        this.$api.regionInf.findAllRelation(param).then(res => {
          //console.log(res);
          res.forEach((item) => {
            item.isin = true ;
            item.name = item.watermetername ;
            item.id = item.watermeterid ;
          });
          this.totalData = res ;
